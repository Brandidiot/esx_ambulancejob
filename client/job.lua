		...
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = _U('ems_menu_vitals'), value = 'vitals'},
					{label = _U('ems_menu_revive'), value = 'revive'},
					{label = _U('ems_menu_small'), value = 'small'},
					{label = _U('ems_menu_big'), value = 'big'},
					{label = _U('ems_menu_putincar'), value = 'put_in_vehicle'},
					{label = _U('ems_menu_pullout'), value = 'pull_out_vehicle'}
			}}, function(data, menu)
				...
				if closestPlayer == -1 or closestDistance > 2.0 then
					...
					elseif data.current.value == 'vitals' then
						local playerPed = PlayerPedId()
						ESX.ShowNotification( "Checking Vitals", false,true,10)
						TaskStartScenarioInPlace(playerPed, 'CODE_HUMAN_MEDIC_TEND_TO_DEAD', 0, true)
						Citizen.Wait(10000)
						TriggerServerEvent('medSystem:checkvitals', GetPlayerServerId(playerPed), GetPlayerServerId(closestPlayer))
					end
				end
			end, function(data, menu)
				menu.close()
